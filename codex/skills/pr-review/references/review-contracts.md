...
After the seven ordered verdicts, include explicit `### <Principle>` finding sections only for principles with retained findings. Every finding that is verified in the current ledger and current epoch appears exactly once under its assigned principle; no dismissed, superseded, candidate, or stale historical state appears. Omit empty principle lists.
