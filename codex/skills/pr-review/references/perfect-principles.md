...
After the ordered verdicts, list retained findings in explicit `### <Principle>` sections. List every retained verified ledger finding exactly once under its assigned principle, list no other ledger records, and omit empty sections. Under each finding, include `Scenario`, `Why`, `Fix`, and `Anchor`, each a single high-level line: a plausible scenario that would produce the issue, what actually produces it, a high-level potential fix, and a `file:line` anchor for a review comment. Keep them terse; derive `Scenario`, `Why`, and `Fix` from the finding's `Claim`, `Impact`, and `AffectedBehavior`, and `Anchor` from its `Scope` or `Evidence` locations.
