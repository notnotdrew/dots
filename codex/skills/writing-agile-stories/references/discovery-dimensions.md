...
Before drafting, gather these six dimensions. If the caller has supplied context (Jira/Linear ticket, codebase research, prior conversation), extract from that. If context is thin, ask the user in plain prose for what's missing. Discovery is open-ended, so avoid forcing the conversation into rigid multiple-choice prompts.
...
Avoid rigid multiple-choice prompts for these. Discovery is open-ended, and constrained options tend to distort the user's answer.
