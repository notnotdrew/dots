...
**Trigger**: They changed their mind about a purchase
**Outcome**: Order cancelled, refund confirmation received
**Constraints**: Only before shipment; returns process applies after shipment
**Failure Modes**: Order already shipped, refund processing failure
...
### Context
Available for orders in "confirmed" or "processing" status that have not
...
**Actor**: Account holder responding to a data subject request
**Trigger**: GDPR or CCPA compliance need
**Outcome**: Complete personal data download in portable format
**Constraints**: Verified account required, 30-day retention window
...
When account holders receive data subject requests, they need to download
a complete copy of their personal data in a portable format. This allows
them to fulfill legal obligations and maintain records independently of
the platform.

### Context
Available to verified account holders with active or recently closed
accounts within the 30-day retention window.
...
- Then they see an estimated completion time
...
- Given an account holder has not verified their email
...
**Trigger**: Forgotten password or account security concern
**Outcome**: Secure password reset and restored account access
**Constraints**: Email verification required, reset link expiration
...
- And they are offered support escalation if the issue is urgent

#### Scenario: Password requirements not met
- Given a user is setting a new password
- When they enter a password that does not meet requirements
- Then they see specific feedback about what is missing
...
```markdown
The system needs a /auth/reset-password endpoint that accepts POST requests
and inserts a token into the password_resets table.
```
