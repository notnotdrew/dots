...
Before drafting, gather these six dimensions. If the caller has supplied context (Jira/Linear ticket, an ADR, a failing build, a flaky-test report, prior conversation), extract from that. If context is thin, ask the user in plain prose for what's missing. Discovery is open-ended, so avoid multiple-choice prompts that force contrived answers.
...
Keep these open-ended. A fixed option list would constrain the answer to choices you invented before understanding the problem.
